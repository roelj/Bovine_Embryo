...
echo "# Merging VCFs"
echo "bcftools concat $1*.vcf.gz > $2BovineSperm_SNPs_Het_Embryo.vcf"
guixr load-profile ~/.guix-profile/ -- <<EOF
	bcftools concat $1*.vcf.gz -O z -o $2BovineSperm_SNPs_Het_Embryo.vcf.gz
	bcftools index $2BovineSperm_SNPs_Het_Embryo.vcf.gz
	bcftools stats $2BovineSperm_SNPs_Het_Embryo.vcf.gz > $2BovineSperm_SNPs_Het_Embryo.stats
